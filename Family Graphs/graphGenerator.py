...
#and uses them to generate graphs of their interconnectivity.
#
#Author: Benjamin Niedzielski (bniedzie@ucla.edu)
#Contributor: Theodora Ng
...
pplAdded = set();                                   #A set to ensure that people are added to a graph only once
linksAdded = set();                                 #A set to ensure that relationships are added to a graph only once
nominaAdded = {};                                   #A list that keeps track of which families have been added to a graph, for node-coloring purposes
...
class Person():
    def __init__(self):
        ...
        self.relatives = [];
        self.bestPosition = ""; #Best political position
        self.gensID = -1; #Each family gets a unique ID for coloring purposes
        self.marked = False; #Marked people will not be added to the graph again

#Parse the file containing names, ids, and dates
...
def parseRelationshipFile():
    ...
    for line in content:
        ...
        #splitline[1] is Person 1/first person in a relationship/connection
        ...
        #splitline[2] is Person 2 in a relationship/connection
...
#Input: person, the Person to print JSON information for
#Input: indent, how far indented the JSON output shoulf start
#Input: model, an int that determines which relative types to consider, with
#0 meaning father, mother, and grandfather, 1 adding in
#son and daughter, 2 addeding spouse, and 3 adding adoptions
#Return: the JSON text for this person and relatives
...
#Given a person and a type of traversal, create a d3 graph for them based off of the template.
def traversePerson(id, mode):
    ...
    indent = 0;
    person = allPeople[id];
    fp = open('graphTemplate.html', 'r');

    if (mode == 0):
        writeFile = 'FamilyConnectivity/' + person.name[0:8] + ' - Simple.html';
        
    if (mode == 1):
        writeFile = 'FamilyConnectivity/' + person.name[0:8] + ' - Simple with Children.html';
        
    if (mode == 2):
        writeFile = 'FamilyConnectivity/' + person.name[0:8] + ' - Simple with Children and Spouse.html';
        
    if (mode == 3):
        writeFile = 'FamilyConnectivity/' + person.name[0:8] + ' - Simple with Children, Spouse and Adopted.html';
...
#Ask user who to run the algorithm on    
